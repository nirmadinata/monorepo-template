...
import { discussionPlugin } from "#/components/editor/plugins/discussion-kit.tsx";
import type { TDiscussion } from "#/components/editor/plugins/discussion-kit.tsx";
import type { TComment } from "#/components/ui/comment.tsx";
...
type BlockDiscussionEntry = NodeEntry<TCommentText | TElement | TSuggestionText>;
...
interface BuildBlockDiscussionIndexOptions {
    ...
    getSuggestionData: (node: TElement | TSuggestionText) =>
        ...
        | undefined;
    getSuggestionDataList: (node: TSuggestionText) => {
        id: string;
        newProperties?: Record<string, unknown>;
        properties?: Record<string, unknown>;
        type: "insert" | "remove" | "update";
...
const getTopLevelPath = (path: Path): Path | null => (path.length > 0 ? path.slice(0, 1) : null);
...
const getSuggestionIds = (
    ...
    if (TextApi.isText(node)) {
        ...
        const updateIds = dataList.filter((data) => data.type === "update").map((data) => data.id);
...
const getInlineSuggestionElementText = (node: TElement) => {
    ...
    if (
        ...
        typeof (node as TElement & { texExpression?: unknown }).texExpression === "string" &&
...
const toResolvedSuggestion = ({
    ...
    const sortedEntries = [...entries].toSorted(([, path1], [, path2]) =>
...
export const buildBlockDiscussionIndex = ({
    ...
    entries.forEach(([node, path]) => {
        ...
        getSuggestionIds(node, getSuggestionDataList, getSuggestionId).forEach((suggestionId) => {
...
const getDiscussionIndex = (editor: PlateEditor, discussions: TDiscussion[], version: number) => {
    const cached = discussionIndexCache.get(editor);

    if (cached && cached.version === version && cached.discussions === discussions) {
        return cached.index;
    }
